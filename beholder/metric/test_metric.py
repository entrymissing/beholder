...
class TestMetric(Metric):
    ...
    def collect(self):
        ...
        for time_window, suffix in zip(self._parameters['time_windows'], self._parameters['suffixes']):
            ...
            data.append(DataPoint(self._base_name + 's_count.' + suffix, time.time(), len(events)))
            data.append(DataPoint(self._base_name + 's_seconds_since.' + suffix, time.time(), min(seconds_since)))
            data.append(DataPoint(self._base_name + 's_short_count.' + suffix, time.time(), short_count))
            data.append(DataPoint(self._base_name + 's_long_count.' + suffix, time.time(), long_count))
        return data
